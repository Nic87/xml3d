...
X3D.main = function() {
    ...
    return {        
        ...
        toggleMode : function(){
            ...
            else if(mode===roomModus)mode=rotateModus; 
		...
		createViewDialog : function(){
			...
			var newVect = xml3dObj.createXML3DVec3();
			newVect.x = 435;
			tmp.setDirection(newVect);*/   // Standard position: x = 0 y = 0 z = 20
			//this.showPosition(tmp); // x= links/rechts y = Hoch/runter z = vor/zurück
			//tmp.position.x += 5; 
			//this.showPosition(tmp);
			//tmp.orientation.w += 1; //Standard orientation="0 0 0 0"
			//console.log('Winkel: ' + tmp.orientation.angle);
			//tmp.orientation.orientation;
			//this.setAxisAngleValue('1');
			//this.resetXml3D();
			...
			v.y = 1;
			v.z = 0;
			var zRotation = xml3dObj.createXML3DRotation();
			zRotation.setAxisAngle(v, 0.1745); //10 Grad
			tmp.orientation = tmp.orientation.multiply(zRotation);
			this.resetXml3D();
        ...
        createElementFromGeneric:function(prefix,genericID, x, y, z){
            log.info('Prefix:'+prefix+':genericID:'+genericID+':');
            var newElement = X3D.gen.insertGenericGroup(prefix,genericID);
        ...
        changeShader:function(shaderID){
            ...
            if(shader){
                for(var i = 0;i<selectedElems.length;i++){
                    ...
                    log.info('Changing Shader of;'+selectedElems[i].getAttribute('id')+' with shader: '+texts[0]+texts[1]);
                }
            }
            this.resetXml3D();
        },
