...
X3D.main = function() {
	...
	return {		
		...
		getElementsOfType:function(type){
			var shaders = document.getElementsByTagNameNS('http://www.xml3d.org/2009/xml3d','shader');
			return shaders;
		},
		...
		createBox:function(id){
			...
			var transename = this.testPositionTransformer(box);
			box.setAttribute('transform','#'+transename);
		...
		moveSelectedObject:function(keyNumber){
			for(var i = 0;i<selectedElems.length;i++){
				...
				switch(parseInt(keyNumber)){
					...
					case 38:{
						if(modus===planeModus)transformer.translation.y += 0.1;
						else if (modus===roomModus)transformer.translation.z += 0.1;
						break;
					}
					case 39:{
						transformer.translation.x += 0.1;
