...
X3D.gen = function() {
    ...
    return {
        ...
        /* "imports" the given generic definition into the current document scope */
        ...
        insertGenericGroup : function(name, searchPrefix, id){
            ...
            if(src!=null){
                ...
                var groupEle = this.slowGetById(xmldoc,id);
            ...
            if(attributes == null){
                ...
                for(var j =0;j<attributes.length;j++){
                    if(this.startsWithDiamond(attributes[j].value)){
                        var newId= this.checkOrCreateDef(xmldoc,attributes[j].value);
        ...
        checkOrCreateDef : function (xmldoc, id) {
            var newId = prefix+this.removeDiamond(id);
            var element = this.slowGetById(document,newId);
            if (element == null) {
                this.insertDefElement(this.slowGetById(xmldoc, this.removeDiamond(id)));
            }
            return newId;
        },
        ...
        /* creates a copy of a given element (XXX: incomplete documentation, feel free to fix)
           XXX: we copy all attributes of the given element; what about IDs? must not happen */
        copyElement : function (element){
            ...
            /* XXX: what does this refer to in this scope? and why do we append? */
        ...
        insertDefElement : function (element) {
            ...
            /* XXX: where does the prefix come from? coding by obfuscation suxx. */
